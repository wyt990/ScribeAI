...
* **Organization 模型**：用户可属于多个组织，每条记录含所属行业与描述
* **人员身份**：`UserOrganization` 中间表记录用户在组织内的**职务**（`jobTitle`）和**具体职责**（`responsibilities`）
* **组织上下文贯穿**：录音时选用组织上下文（Draft 的 `orgId`），转正为正式会话后带入 Transcript，生成纪要时作为组织身份注入 LLM 提示词（Summary 的 `orgId`）
* **默认组织**：用户可设置默认组织，新建录音时自动选用
...
| **UI/样式**          | Tailwind CSS v4，shadcn/ui（new-york 风格） |
| **AI**               | Gemini 2.5 Flash / DeepSeek Chat，Agent Skills 纪要模板，Deepgram nova-2 / OpenAI 兼容 ASR |
| **Skills**           | 双层级模板体系（SummarySkill + SummaryTemplate），AI 生成/编辑/预览/导入导出模板 |
| **实时通信**         | Socket.io Client / Server          |
| **状态管理**         | Zustand                             |
| **录制**             | MediaRecorder API + RNNoise AudioWorklet 降噪 |
| **VAD**              | Silero VAD（@ricky0123/vad-web），ONNX Runtime Web WASM   |
...
音频处理管线 (`buildAudioPipeline`) 在录音时对原始音频流依次应用以下处理：

| 环节 | 技术 | 说明 |
|------|------|------|
| **增益控制** | `GainNode` | 范围 0–3×，步长 0.2 |
| **自动增益** | RMS 动态调节 | 目标 RMS 0.16，每 120ms 调整，平滑系数 0.18 |
| **AI 降噪** | RNNoise AudioWorklet | 实时降噪，提升小声说话识别率 |
| **响度压缩** | `DynamicsCompressorNode` | threshold -22dB，ratio 8，平滑输出电平 |

* 增益与降噪设置持久化至 `localStorage`（`scribeai-audio-settings`）
* 录音中已锁定不可修改，停止后可调整

### 📊 音量指示器

录音进行时在控制面板下方显示 28 段式音量条：
* 绿（0–55%）→ 黄（55–80%）→ 红（80–100%）
* 通过 `AnalyserNode` 采集 RMS 值，经指数平滑（系数 0.35/0.65）渲染
* 支持无障碍 ARIA `role="meter"` 属性

### ⏱️ 录音时长

录音中在实时转录卡片标题栏显示累积时长，暂停期间暂停计时：
* 使用 `useRecordingDuration()` hook 跟踪（`activeStartRef` + `accumulatedRef`）
* 格式：`23秒` / `1分23秒` / `1小时43分23秒`
